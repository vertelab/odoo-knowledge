...
Wysiwyg.include({
     ...
     */
    _insertTemplate() {
        const templateBlock = $(QWeb.render('knowledge_powerbox_option.abstract_behavior', {
        ...
        dialog.on('save', this, article => {
            if (article) {
                const articleLinkBlock = $(QWeb.render('knowledge_powerbox_option.wysiwyg_article_link', {
                    href: '/document/article/' + article.id,
    ...
    _onMediaDialogSave(params, element) {
        if (element.classList.contains('o_is_knowledge_file')) {
            ...
            const fileBlock = $(QWeb.render('knowledge_powerbox_option.abstract_behavior', {
